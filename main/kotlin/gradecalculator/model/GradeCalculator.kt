...
abstract class GradeCalculator(
    ...
    protected val categories: MutableList<GradeCategory> = mutableListOf()
    ...
    override fun calculateGrade(): Double {
        val validCats = categories.filter { it.getEffectiveAverage() != null }
    ...
    override fun getLetterGrade(): String {
        ...
        return if (pct == 0.0 && categories.all { it.grades.isEmpty() }) "N/A"
    ...
    fun addCategory(category: GradeCategory) { categories.add(category) }
    fun removeCategory(id: String) { categories.removeIf { it.id == id } }
    fun getCategories(): List<GradeCategory> = categories.toList()
    fun getTotalWeight(): Double = categories.sumOf { it.weight }

    fun getNeededScore(targetPct: Double): Double {
        val earnedWeight = categories.filter { it.getEffectiveAverage() != null }.sumOf { it.weight }
        val pendingWeight = categories.filter { it.getEffectiveAverage() == null }.sumOf { it.weight }
        if (pendingWeight == 0.0) return -1.0
        val earnedSum = categories.filter { it.getEffectiveAverage() != null }
